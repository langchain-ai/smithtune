...
def _action(message):
    """Compare exact visible content and calls across the two known wire formats."""
    ...
    if calls:
        ...
        for call in calls:
            if "function" in call:
                ...
                content.append({"type": "tool_call", "id": call.get("id"), "name": fn.get("name"), "args": args})
            else:
                content.append({"type": "tool_call", "id": call.get("id"), "name": call.get("name"), "args": call.get("args")})
    return content
...
def capture_bindings(messages, runs):
    ...
    for index, message in enumerate(messages):
        ...
        try:
            if _action(message) != _action(output):
