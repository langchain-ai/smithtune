...
def _parser() -> argparse.ArgumentParser:
    ...
    for name, help_text in {
        ...
        if name == "pull":
            ...
            command.add_argument("--target-count", type=int, help="desired structurally usable trajectories before council review (default: 100)")
            command.add_argument("--max-candidates", type=int, help="maximum distinct candidates to examine while backfilling exclusions (default: 1000, maximum: 2000)")
        if name == "push":
        ...
        if args.command == "doctor":
            ...
            if args.dataset_command == "publish-splits":
                ...
                options = {key: getattr(args, key, None) for key in (
                    "workspace_id", "project_id", "start_time", "end_time", "filter", "target_count", "max_candidates",
