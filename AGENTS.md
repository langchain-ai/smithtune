...
- Preparation publishes dataset splits by default, including with `--no-fetch`. Replay verifies that pinned dataset version before paid work and returns LangSmith experiment links. Use `--no-sync-splits` and replay's `--no-langsmith` for local-only work; see the README for publishing saved results later.
...
- LangSmith replay experiments group independent next-action predictions by source conversation. `teacher_agreement` is per action; `trajectory_teacher_agreement` is the conversation's mean. Upload failures resume from saved predictions and judgments after owned serving resources have been closed.
