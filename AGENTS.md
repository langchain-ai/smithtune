...
- LangSmith replay experiments group independent next-action predictions by source conversation. `teacher_agreement` is per action; `trajectory_teacher_agreement` is the conversation's mean. Experiment metadata includes the parent smithtune run ID and selected checkpoint epoch when saved training provenance is available. Publish completed action children in the background and show the comparison link before generation. Post each finished conversation parent and its aggregate score once all selected actions are complete; never PATCH run outputs or publish partial/open parents. On resume, reject legacy partial/open parents before paid work without overwriting them or replacing experiments. Upload failures resume from saved predictions and judgments; release owned serving resources before the final publication wait.
