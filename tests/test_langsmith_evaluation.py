"""Exercise the real SDK evaluator with an in-memory LangSmith service boundary."""
...
class MemoryClient(Client):
    ...
    def create_feedback(self, run_id=None, key="unnamed", *, score=None, comment=None, **kwargs):
        if key == self.drop_feedback:
            raise ConnectionError("feedback unavailable")
        item = SimpleNamespace(id=uuid4(), run_id=UUID(str(run_id)), key=key, score=score, comment=comment)
...
def prepared(tmp_path, monkeypatch):
    ...
    manifest = dataset.prepare_dataset(WORKSPACE, DATASET, DEFAULT_MODEL, data, fetch=False,
...
def test_real_sdk_attaches_child_and_root_feedback_and_resumes(prepared, tmp_path):
    data, manifest, client = prepared
    calls = []
...
def test_feedback_upload_failure_does_not_repeat_inference_or_successful_feedback(prepared, tmp_path):
    ...
    with pytest.raises(PipelineError, match="feedback upload is incomplete"):
