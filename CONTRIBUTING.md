...
The default council is read from the packaged `triage_prompts/default_council.json`;
CLI and Python defaults must agree. The coordinator and judge prompts in
`triage_prompts/` are internal; operator guidance belongs in
`skills/smithtune/SKILL.md`. Terra uses OpenAI Responses
...
distributions must include the workflow skill and triage prompts as package data.
